 ...
 * @todo PUT
 */
export let RequestMethod = /*#__PURE__*/function (RequestMethod) {
...
const _startHttpService = async (port, cb) => {
  ...
  if (Platform.OS === 'android') {
    lsnr_handle.push(DeviceEventEmitter.addListener(`${serviceId}-data`, event => {
      onData(event);
    }));
    lsnr_handle.push(DeviceEventEmitter.addListener(`${serviceId}-error`, async event => {
      await onError(event);
    }));
...
const _createTcpConnection = async (param, onData) => {
  ...
  if (Platform.OS === 'android') {
    lsnr_handle.push(DeviceEventEmitter.addListener(`${connId}-data`, event => {
      onData(event);
    }));
    lsnr_handle.push(DeviceEventEmitter.addListener(`${connId}-error`, async event => {
      await onError(event);
