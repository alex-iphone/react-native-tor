...
const TorBridge = _reactNative.NativeModules.TorBridge;
...
var _default = function _default() {
  ...
  const _handleAppStateChange = async nextAppState => {
    if (startDaemonOnActive && lastAppState.match(/background/) && nextAppState === 'active') {
      const status = _reactNative.NativeModules.TorBridge.getDaemonStatus();
  ...
  const startIfNotStarted = () => {
    if (!bootstrapPromise) {
      bootstrapPromise = _reactNative.NativeModules.TorBridge.startDaemon(bootstrapTimeoutMs);
    }
  ...
  const stopIfRunning = async () => {
    ...
    await _reactNative.NativeModules.TorBridge.stopDaemon();
  };
