...
const _startHttpService = async (port, cb) => {
  ...
  if (_reactNative.Platform.OS === 'android') {
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener("".concat(serviceId, "-data"), event => {
      onData(event);
    }));
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener("".concat(serviceId, "-error"), async event => {
      await onError(event);
    }));
...
const _createTcpConnection = async (param, onData) => {
  ...
  if (_reactNative.Platform.OS === 'android') {
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener("".concat(connId, "-data"), event => {
      onData(event);
    }));
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener("".concat(connId, "-error"), async event => {
      await onError(event);
...
var _default = ({
  ...
  const _handleAppStateChange = async nextAppState => {
    if (startDaemonOnActive && lastAppState.match(/background/) && nextAppState === 'active') {
      const status = _reactNative.NativeModules.TorBridge.getDaemonStatus(); // Daemon should be in NOTINIT status if coming from background and this is enabled, so if not shutodwn and start again
