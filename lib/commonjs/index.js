...
exports.default = exports.RequestMethod = void 0;
var _reactNative = require("react-native");
var _async = require("async");
// const TorBridge: NativeTor = NativeModules.TorBridge;
const TorBridge = global.TorBridge;
...
const _createHiddenService = async (hiddenServicePort, destinationPort, secretKey) => {
  let hiddenServiceJson = await _reactNative.NativeModules.TorBridge.createHiddenService(hiddenServicePort, destinationPort, secretKey !== null && secretKey !== void 0 ? secretKey : '');
...
const _deleteHiddenService = async onion => {
  await _reactNative.NativeModules.TorBridge.deleteHiddenService(onion);
  return true;
};
const _startHttpService = async (port, cb) => {
  let serviceId = await _reactNative.NativeModules.TorBridge.startHttpHiddenserviceHandler(port);
  const lsnr_handle = [];
  /**
  ...
  if (_reactNative.Platform.OS === 'android') {
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener(`${serviceId}-data`, event => {
      onData(event);
    }));
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener(`${serviceId}-error`, async event => {
      await onError(event);
    }));
  } else if (_reactNative.Platform.OS === 'ios') {
    const emitter = new _reactNative.NativeEventEmitter(_reactNative.NativeModules.TorBridge);
  ...
  const close = () => {
    ...
    return _reactNative.NativeModules.TorBridge.stopHttpHiddenserviceHandler(serviceId);
  };
  return {
    close
  };
...
const _createTcpConnection = async (param, onData) => {
  ...
  const connId = await _reactNative.NativeModules.TorBridge.startTcpConn(target, connectionTimeout);
  let lsnr_handle = [];
  /**
   * Handles errors from Tcp Connection
   * Mainly check for EOF (connection closed/end of stream) and removes lnsers
  ...
  const onError = async event => {
    if (event.toLowerCase() === 'eof') {
      console.warn(`Got to end of stream on TcpStream to ${target} having connection Id ${connId}. Removing listners`);
      try {
        await close();
      } catch (err) {
  ...
  if (_reactNative.Platform.OS === 'android') {
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener(`${connId}-data`, event => {
      onData(event);
    }));
    lsnr_handle.push(_reactNative.DeviceEventEmitter.addListener(`${connId}-error`, async event => {
      await onError(event);
  ...
  const write = msg => _reactNative.NativeModules.TorBridge.sendTcpConnMsg(connId, msg, writeTimeout);
  const close = () => {
    lsnr_handle.map(e => e.remove());
    return _reactNative.NativeModules.TorBridge.stopTcpConn(connId);
  };

  /**
   * Wrap write with a JS queue for non V8 engines
   */
  const writeMessageQueue = (0, _async.queue)(async (payload, cb) => {
    const {
      msg,
      res,
      rej
...
const createTcpConnQueue = (0, _async.queue)(async (payload, cb) => {
  const {
    param,
    res,
    rej
 ...
 */
const createTcpConnection = function () {
...
var _default = function _default() {
  ...
  if (numberConcurrentRequests > 0) {
    requestQueue = (0, _async.queue)(async (task, cb) => {
      const {
        res,
        rej,
        request
  ...
  const stopIfRunning = () => {
    ...
    return new Promise((res, rej) => TorBridge.stopDaemon(r => r === 1 ? res(true) : rej('Invalid return')));
  };
  ...
  return {
    async get(url, headers) {
      let trustSSL = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
      await startIfNotStarted();
      return new Promise((res, rej) => TorBridge.request(url, RequestMethod.GET, '', headers || {}, trustSSL, result => {
    ...
    async post(url, body, headers) {
      let trustSSL = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
      await startIfNotStarted();
      return await onAfterRequest(await requestQueueWrapper(() => TorBridge.request(url, RequestMethod.POST, body, headers || {}, trustSSL, res => {
    ...
    async delete(url, body, headers) {
      let trustSSL = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
      await startIfNotStarted();
      return await onAfterRequest(await requestQueueWrapper(() => TorBridge.request(url, RequestMethod.DELETE, body || '', headers || {}, trustSSL, res => {
